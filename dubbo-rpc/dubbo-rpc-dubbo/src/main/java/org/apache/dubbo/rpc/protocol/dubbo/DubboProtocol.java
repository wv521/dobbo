...
public class DubboProtocol extends AbstractProtocol {
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        URL url = invoker.getUrl(); // dubbo://169.254.55.8:20880/org.apache.dubbo.demo.DemoService?anyhost=true&application=demo-provider&bind.ip=169.254.55.8&bind.port=20880&deprecated=false&dubbo=2.0.2&dynamic=true&generic=false&interface=org.apache.dubbo.demo.DemoService&methods=sayHello,sayHelloAsync&pid=14260&qos.port=22222&release=&side=provider&timestamp=1593484742994

        // export service.
        String key = serviceKey(url);  // key ====》接口地址路径：dubbo端口  org.apache.dubbo.demo.DemoService:20880

        // 构建一个DubboExporter 包装了8个filter的实现类对象
        ...
        // 暴露完服务后开启通讯能力，以便供远程调用的能力---通过netty/tcp 协议 来提供
        openServer(url);
    ...
    private void openServer(URL url) {
        // find server.
        // 获取一个通讯的服务地址（netty通讯）
        String key = url.getAddress();  // key ： 本机ip+dubbo端口
        ...
        if (isServer) {
            ...
            if (server == null) {
                synchronized (this) {
                    ...
                    if (server == null) {
                        // 不存在则创建一个并加入到缓存  此处会创建一个用于通讯的ExchangerServer，加入到存放通讯服务的缓存serverMap中
    ...
    private ProtocolServer createServer(URL url) {
        ...
        // 将ExchangerServer 包装成一个DubboProtocolServer
    ...
    @Override // serviceType：服务接口
    public <T> Invoker<T> protocolBindingRefer(Class<T> serviceType, URL url) throws RpcException {
        optimizeSerialization(url);

        // create rpc invoker. 构建一个远程调用的dubboInvoker对象
        // getClients(url) 获取客户端实例
    ...
    private ExchangeClient[] getClients(URL url) {
        ...
        if (connections == 0) { // 如果没有配置connections，则为共享连接
