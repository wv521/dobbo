...
public abstract class FailbackRegistry extends AbstractRegistry {
    ...
    private void addFailedRegistered(URL url) {
        // 先从缓存中获取，如果有直接返回不做任何操作
        FailedRegisteredTask oldOne = failedRegistered.get(url);
        if (oldOne != null) {
            return;
        }
        // 如果缓存中不存在，通过构建一个定时任务去定时重试，默认每隔5s执行
        FailedRegisteredTask newTask = new FailedRegisteredTask(url, this);
        // map.putIfAbsent(key,value) --->如果key对应的value存在就返回value，如果不存在就添加进去并返回null。
    ...
    public void register(URL url) {
        ...
        try {
            ...
            // 向注册中心注册服务（写节点）。以zookeeper注册中心为例时其实现类：ZookeeperRegistry
            doRegister(url);
        } catch (Exception e) {
            Throwable t = e;
            ...
            // 将注册失败的节点，注册到注册失败的集合中，以便进行尝试重新注册。
    ...
    public void subscribe(URL url, NotifyListener listener) {
        super.subscribe(url, listener); // 往本地存储映射关系
        removeFailedSubscribed(url, listener);
        try {
            // Sending a subscription request to the server side
            // 发送订阅请求。ZookeeperRegistry.doSubscribe(url, listener);
            doSubscribe(url, listener);  // URL : 暴露的接口地址/configurators
        } catch (Exception e) {
            Throwable t = e;
