...
public class ReferenceConfig<T> extends ReferenceConfigBase<T> {
    ...
    public synchronized void init() {
        ...
        Map<String, String> map = new HashMap<String, String>();  // 拼装消费者配置参数
...
//        "timestamp" -> "1591065064285"
    ...
    private T createProxy(Map<String, String> map) {
        if (shouldJvmRefer(map)) {  // 本地服务调用
            URL url = new URL(LOCAL_PROTOCOL, LOCALHOST_VALUE, 0, interfaceClass.getName()).addParameters(map);
            // 实现类
            // 1.ProtocolFilterWrapper.refer()
            // 2.ProtocolListenerWrapper.refer()
            // 3.RegistryProtocol.refer() 最终实现类
            invoker = REF_PROTOCOL.refer(interfaceClass, url);  // 服务引入
            ...
            if (urls.size() == 1) {  // 单个服务
                // 实现类
                // 1.ProtocolListenerWrapper.refer()
                // 2.ProtocolFilterWrapper.refer()
                // 3.RegistryProtocol.refer() 最终实现类
                // interfaceClass----> 引用的接口
                invoker = REF_PROTOCOL.refer(interfaceClass, urls.get(0));
            } else {  // 多服务
                List<Invoker<?>> invokers = new ArrayList<Invoker<?>>();
                URL registryURL = null;
                for (URL url : urls) { // 多个服务循环导出
